...
import { Play, RotateCcw, Download, Loader2, Save, PlayCircle } from 'lucide-react';
import { initializePopulation, runSearch, addActivity, updateKPIs, fetchModelCode } from '../data/Data';
...
export const SearchPage = () => {
  ...
  const addLog = (message) => {
    ...
    setLogs(prev => [...prev, { id: `${Date.now()}-${logCounter.current}`, message, timestamp: new Date().toLocaleTimeString() }]);
  };

  const handleInitialize = async () => {
    setIsInitializing(true);
    addLog('[INFO] Sending initialization request to backend...');

    try {
      const initResult = await initializePopulation(config);
      result.current = initResult;
      (initResult.logs || []).forEach(log => addLog(log));
      ...
      addActivity('Population initialization failed', 'warning');
    } finally {
  ...
  const handleStartSearch = async () => {
    ...
    try {
      ...
      if (result.current.kpis) {
        updateKPIs(result.current.kpis);
      }
  ...
  const handleSimulateBuild = async () => {
    ...
    try {
      const bestModelId = result.current?.bestModel?.modelId || result.current?.population?.[0]?.modelId;
      const pythonCode = bestModelId
        ? await fetchModelCode(bestModelId)
        : '# Run a search first to generate model code from the backend.\n';
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `${bestModelId || 'monas_model'}_${Date.now()}.py`;
      ...
      alert('Model code generated from backend!');
    } catch (error) {
      addLog(`[ERROR] Build failed: ${error.message}`);
      addActivity('Build simulation failed', 'warning');
    }
  };
      ...
      <div className="data-card">
        <div className="flex flex-wrap gap-3">
          <button
            ...
            disabled={initialized || isRunning || isInitializing}
          ...
          >
            {isInitializing ? <Loader2 size={18} className="animate-spin" /> : <RotateCcw size={18} />}
            {isInitializing ? 'Initializing...' : 'Initialize Population'}
          </button>
